...
## 2026-09-15（续）《向着太阳》用 QQ音乐 320k 版独立复核 + 工具参数化

**源确认（用户要求用 QQ音乐版本）**：我此前的独立复核**本就用的是 QQ 版**，有据可查——
- 向着太阳重测\音源\向着太阳\001gW7th0K2rgc_320k.mp3（**320 kbps / 5:18.5**）为分离源；
  对应用 分析结果\向着太阳_官方320k_f0.csv（27439 帧），站点口径键亦为 向着太阳_官方320k ✅
- ⚠️ 注意 qqmusic下载\向着太阳.mp3 **仅 128 kb/s**（非 320k），不得当官方 320k 用
- B站版（ili_向着太阳.wav，5:08.62）**未用于本次复核**

**独立复核结论（不沿用 09-09 结论）**
| 时刻 | 读数 | 1f0 占比 | 判定 |
|---|---|---|---|
| **t=189.5s** | **91.5Hz（≈F#2）** | **0.625** | ✅ 真音（证据最强） |
| t=188.9s | 95.5Hz | 0.731 | ✅ 真音 |
| t=33.42s（现行口径） | 97.8Hz（G2） | 0.444（2f0=0.512） | ⚠️ 四个 G2 候选里证据最弱 |
| 61.9Hz（YIN 全曲最低帧） | — | **0.000** | ❌ 伪影 |
- 人声轨 60–100Hz 能量占比仅 **0.1497%** → 超低音（43–75Hz）是**伴奏低音器乐**，非其人声
- **建议**：本曲最低稳定音由 G2 97.8Hz 修正为 **F#2 91.5Hz**（待确认后改口径）
- **传记价值**：他在本曲的低音到 F#2/G2，**并未触及自身极限**（专辑最低 B1 61.5Hz）——
  恰印证「低音为作品服务、不刻意刷极限」

**工具修复**
- 场次音域报告.py：SONG 及其派生的 OUT_JSON/OUT_MD 原写死「让她降落」
  → 全部参数化（加 --song，默认向后兼容）；输出改为 <SONG>_四版实测.md/.json
- 场次音域一键.py：批量专辑音域.py 路径 ANALYSIS→ROOT（原报 can't open file，整链停在第 3 步）
- 场次音频准备.py：SONG 加 --song

## 2026-09-15（续）《友谊地久天长》HighC 实测 + 高音区倍频锁错（新发现）

- **《友谊地久天长》四巡（非三巡）南京 2024-01-27 实测**：确实唱到 **C5 (HighC 523.25Hz)，持续约 4.0s**。
  该曲此前不在任何声学层；本次入库（汇总 55→57 首 / 19→20 组）。
- **⚠️ 新方法论发现：高音区「倍频锁错」** —— 工具原报 `high_stable = C6 1059.3Hz` 是伪影。
  分离人声谐波列显示 **1f0(523) 弱、2f0(1046) 与 3f0(1569) 强**，而 1569=3×523
  → 谐波系列以 523 为基 → 该音是 C5，YIN 锁到了 2 次谐波。
  · 低音区 1/3 次谐波锁错（报低）《向着太阳》：CREPE/YIN>1.5 **能抓**
  · **高音区 2 倍频锁错（报高）《友谊地久天长》：比值<1.5，现有闸门抓不到** ← 盲区
  · 建议给管线加对称的「高音区闸门」（2f0 强于 1f0 且 3f0 强于 2f0 → 报高需降八度）
- **修工具两处**：`场次音域一键.py` 的 `批量专辑音域.py` 路径错（ANALYSIS→ROOT，导致整链停在第 3 步）；
  `场次音频准备.py` 的 `SONG` 写死「让她降落」→ 加 `--song`。
