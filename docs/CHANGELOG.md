...
## 2026-09-15（续）人耳听辨 6 条定案 + 复核闭环（stage.html 可见）

**用户人耳听辨结论**：6 条「待复核（谐波列待判）」全部确认为**王晰本人演唱**
（友谊地久天长 G#2/A2、亲密爱人 D#2、像雾像雨又像风 E2、祝我们随遇而安 F#2、让她降落 F#2）。

**认知修正**：机器判「基频缺失（待判）」的 6 条人耳全为真音；而机器判 1f0 强的 C6 人耳否决。
→ **基频强弱本身不构成真伪判据**，必须人耳或同源交叉定案。

**闭环实现**
- `data/listening_verdicts.json` 听辨台账（7 条）→ `轨迹\生成巡演现场报告.py` 读账
  → 命中 tag 记 `a3_final_note = "人耳确认（听辨）"`（证据级别最高，覆盖机器判定）
- 复核收口：筛查「成立」→「谐波列复核通过」28 条；「待判」→「待复核（谐波列待判）」6 条
  （此前显示「未复核」= 跑了不留痕，已如实化）
- 复核分布（50 条）：人耳确认 6 ｜ 谐波列复核通过 33 ｜ 双引擎一致 5 ｜ 待复核 3 ｜ 参考类 2

**新增听辨样本**：`音域分析\听辨样本\` 16 个 wav + 2 张频谱图 + 清单 json

**操作中心 156 → 158 → 160**：新增 159 听辨台账回填 / 160 听辨定案后重建。

## 2026-09-15（续）⚠️ 外部声学方案的记录缺口（用户指出）+ C6 听辨样本

**缺口**：`demucs 4.1.0`、`torchcrepe`、`faster_whisper` 确在实际使用，
但 **repo 内全部 .py/.html/.md 对 demucs / CREPE / Praat / VRP / Schutte 的记载为零**；
记录只存在于本地工作目录（13 个文件），从不公开。而 doc1 要求的
「工具与开源项目」区块与 4 条文献著录（Schutte & Seidner 1983 / Jadoul 2018 /
Kim 2018 / Défossez 2019）**均未做**。`parselmouth`（Praat）、`opensmile` 未安装。

**C6 听辨样本已产出**（`音域分析\听辨样本\`）：4 个时间点 × 混音/人声轨 = 8 段小音频
+ 2 张频谱图，供人耳定案。

**读谱图新发现**：t≈158s 人声轨呈连续谐波带、**间距约 250Hz**
（250/500/750/1000/1250/1500/1750）→ 若该间距为真基频，则真基频 ≈**250Hz(B3)**，
YIN 报的 1059Hz ≈ **4 倍** → **疑似 4 倍频锁错**（闸门目前只测 2 倍）。
⚠️ 谱图可能混有共振峰成分，需数值确认。

**新增纪律**（用户明确要求，此后自动执行）：
1. 凡实质性修改必须写入 `temp/运维备忘_20260907.md`
2. 凡实质性修改必须集成进 `操作中心.bat`（新脚本进菜单 / 改动脚本确认可达）
   —— 不需提醒，为项目默认纪律。

## 2026-09-15（续）高音区闸门挂进主管线 +《音乐漫谈》16 篇入库

**高音区闸门**：`音域分析\批量专辑音域.py` 新增 `gate_high_register()`，在逐帧 F0 之后调用。
命中判据：分离人声上该高音时刻 **f0/2 占 >20% 且 1.5×f0 占 >15%** → 谐波系列以 f0/2 为基
→ 判定「2 倍频锁错（YIN 报高一个八度）」，`high_stable` 改为 f0/2 并写 `high_gate`（原值不删）。

**首测未在《友谊地久天长》触发**：闸门取「f0≈报告值」的中位时刻（t≈158s），
该时刻 1059Hz 确为最强分音 → 判"真音"正确；先前问题的 **C5 持续段在 t≈220s**，属另一时刻。
→ **遗留**：闸门仅覆盖"报告值自身的时刻"，未覆盖"同曲不同时刻读数性质不同"；
该曲 C6 读数可信度**仍未决**（男低音唱到 C6 不合理，需人工听辨）。

**《王晰的音乐漫谈》**（15 集 + 1 金句视频）已下载转写：**16 篇 / 9,585 字**。
系列标题即"他的音乐观"目录；其中 **p05 低音怎么练**（"低音高着唱，高音低着唱"／
"低音带着头腔、频率宽"／"低音特别容易唱得很笨重"）、**p06 一个八度**、
**p09 咬字看作品需要**、**p14 用低音表达深沉细腻的情感** 四条可直接进
「音乐理解」专题与「自述 vs 实测」诚信档案。

## 2026-09-15（续）操作中心扩到 158 项 + 反向检查扩到「音域分析\轨迹」

**核查发现的缺口**：本会话新建的两个真工具 —— `轨迹\高音区倍频复核.py` 与
`轨迹\低音复核_谐波列.py` —— **都没进操作中心菜单**；而 `audit_ops_coverage` 却报
「未登记 0」，因为它的反向检查**只扫 `project_b/`、`tools/`、根目录**，
**覆盖不到 `音域分析\轨迹\`**（96 个脚本）。这是检查本身的盲区。

**修复**
1. **操作中心 156 → 158**（新增 V 组）：
   - 157 低音谐波列复核（原始混音谐波列；1f0/3f0 缺失即判次谐波）
   - 158 高音区倍频复核（对称闸门：抓 YIN 锁 2 次谐波「报高八度」）
2. **反向检查扩目录**：新增 `TRAJ_DIR` + `TRAJ_EXEMPT`（73 个一次性诊断豁免）并接进
   `reverse_check`（含站外路径 `relative_to` 防御）。
   → 覆盖面 **可运行脚本 150 → 173**；未登记仍 **0**。
   → 今后「轨迹」下新增真工具未进菜单会**自动报警**。
3. **项数同步**：`AGENTS.md` 156→158 项；维护者预设 0–156 共 157 → **0–158 共 159 项**。

**踩坑记录**：向 .bat 插入文本时误写 `\r\n`，而文件本身已用 CRLF、Python 读取时已归一，
→ 生成 26 处 `\r\r` 双回车。**`audit_bat` 当场报错拦下**（"会让 goto 目标带上 CR 而失败"）。
改用 `\n` 后通过 —— 审计链再次证明了价值。

审计：8 项全绿（bat / ops_coverage / pipeline / caliber / preset / jsonld / nav / index_integrity）。

## 2026-09-15（续）🔒 声学值抗回退：PIN 落到生成器输入层 + 审计加防回退守卫

**问题（用户指出）**：「一定要让显示的为准确，不要一运行程序又回到原来错误值」。

**根因**：`data/archive_vocal_albums.json` 派生自 `批量专辑音域.py` 重算的 `*_stats.json`，
而该重算是**非确定性的** —— 《无人岛上》同一素材三次分析分别跑出
**D#2 77.9 / D2 74.2 / D2 75.4**。此前的修正只写在**派生产物**里，
任何人跑一次「批量专辑音域.py --force」+「生成专辑音域报告.py」，
站点就会**静默回退**到错误值。而 `生成专辑音域报告.py` 里**没有任何覆盖表**
（MANUAL / SUPPRESS / PIN 全无）。

**修复（两层）**
1. **生成器输入层加 PIN**（`音域分析\生成专辑音域报告.py`）：
   新增 `PIN` 覆盖表，在层级过滤后、统计前套用；与 `generate_vocal.py` 的
   `MANUAL_REVIEW / SUPPRESS` 同一纪律——**每条必须有证据，无证据禁止改写**。
   首条：`("回望", "无人岛上") → low D2 75.4Hz`，附谐波列证据与三次实测差异。
   → 已验证：重跑生成器后站点仍为 **D2 75.4**。
2. **审计加防回退守卫**（`project_b\audit_caliber.py`）：
   对已在输入层锁定的关键声学值做硬校验，不符即 `[FAIL] 声学值回退` 并 `exit 1`。
   → 已验证双向：正常 `exit 0`；把站点值篡改为 D#2 77.9 后 `exit 1` 并打印原因；
     还原后恢复 `exit 0`。

**为什么审计守卫不可省**：PIN 只管住"生成器重跑"这一条路径；
若有人直接改 JSON、或换了另一条派生路径，PIN 不会触发。
守卫的价值在于——**回退必须被人看见，而不是悄悄上线**。

**已锁定的值**
| 项 | 锁定值 | 落在哪层 |
|---|---|---|
| 《无人岛上》最低音 | D2 75.4Hz | `生成专辑音域报告.py` 的 `PIN` + 审计守卫 |
| 《向着太阳》最低稳定音 | G2 97.8Hz（证据实例 t=188.9s） | `generate_vocal.py` 的 `MANUAL_REVIEW`（原有） |
| 《渔光曲》稳定音 | D#2 79.2Hz（B1 降触达） | `generate_vocal.py` 的 `MANUAL_REVIEW`/`SUPPRESS`（原有） |

## 2026-09-15（续）低音复核回扫（判据无误）+ 高音区倍频复核工具上线

**一、回扫结论：项目自有判据一直是对的，问题只在 1 个读数**
用项目自有 `轨迹\低音复核_谐波列.py`（其判据本就区分「1/2 次谐波 / 1/3 次谐波 / 基频缺失（待判）」，
且用**固定 ±4Hz 窗**而非百分比）回扫现场层：

```
复核素材 47 条；低音读数 183 个；判定不成立/危险 1 个
  四巡南京20240127_BV1pc411v7gU  A2 111.1Hz @57.887s  1/2 次谐波
    1f0 差 -43.6dB、奇数倍全空（真值≈2×）
```

→ **182/183 成立**。这纠正了我此前的判断：**我那些"1f0 占比 = 0.000"的告警，
  是我临时脚本的 bug（8192 点 FFT 在 55Hz 处 ±3% 窗窄于一个 bin）+ 判据混同所致，
  项目工具本身没错、无需大改。**
→ 唯一被标出的是**本次新入库的《友谊地久天长》**：其 A2 读数（t=57.887s）实为 1/2 次谐波，
  真值≈A3。**该曲的"最低稳定音"需按此修正后再对外**。

**二、新增 `轨迹\高音区倍频复核.py`（高音区闸门，对称于低音区）**
- 动机：高音区基频常弱、2 次谐波强，YIN 会锁到 2 次谐波**报高一个八度**
  （《友谊地久天长》真 C5 523Hz 报成 C6 1069Hz）；低音闸门（CREPE/YIN>1.5）**抓不到"报高"**。
- 做法：**复用** `低音复核_谐波列.py` 里已验证正确的 `ladder()/judge()/longest_run()`，
  只把被检查目标由「最低音」换成「最高音」——不重造判据。
- 用法：`python -X utf8 高音区倍频复核.py [--song 曲名] [--write]`
- 首跑（友谊地久天长 2 版）：C6 1059.3Hz / B5 1005.1Hz 均判**「待判」**（该时刻 1f0 即最强分音，
  读数成立），**未标为锁错**；而先前实测的 C5 持续段（t≈220s）是另一时刻，
  该处 1f0(523) 弱、2f0(1046) 强 —— **同一首歌不同时刻结论不同，须按时段分别判定**。

**三、待办**
- 《友谊地久天长》最低稳定音按 1/2 次谐波结论修正
- `data/vocal_lineage.json`（音域脉络）—— 本轮未做

## 2026-09-15（续）A1 练声精测 + 发现自身谐波列判定的分辨率 bug 与判据缺陷

**A1 练声（BV1xNqhYhEqh，39s）精测**：稳定音 **B1 62.0Hz**（18/42 音符合格）；
`f0_min = 55.1Hz (A1)` 仅 **6 帧 / 约 0.1s**（t=4.3/32.7/33.3/34.1/35.9/36.0），
**远未过「极值音本身 ≥0.2s」门槛** → 归**触达级 / 练声场景**，不作能力依据。
（标题"最低音达到 Low C 下面的 A"方向可信，但不构成稳定音。）

**⚠️ 同时发现并修正两处自身方法问题**：
1. **分辨率 bug**：旧判定用 `8192 点 FFT + f0 的 ±3% 取窗`。8192 点 @44100Hz 的 bin 宽 = 5.38Hz，
   而 55Hz 处 ±3% 仅 ±1.65Hz → **窗口窄于一个 bin**，低音处系统性算出 1f0≈0。
   → 改用 `32768 点 FFT`（bin 1.35Hz）+ **固定 ±6Hz 窗**。
2. **判据缺陷**：旧判据把两种情形混为一谈——
   - **次谐波锁错**：只有 **3f0/6f0** 有峰（2f0、4f0 皆无）
   - **缺失基频**（真音但基频弱）：**3f0 与 4f0 都有峰**（如 220 = 4×55）
   → **必须看 4f0 是否存在**来区分。旧法把两者都判成"锁错"。
3. **影响**：本日若干"1f0=0.000 → 次谐波锁错"的判定需用新判据复检，已列入待办。

**《向着太阳》口径**：数值**维持 G2 97.8Hz 不变**（原拟改 F#2 91.5Hz 的建议**已收回**——
F#2 那点全曲仅 28 帧且不连续，系颤音下摆而非独立音符）；已把证据实例由 t=33.4s（1f0=0.444）
换为 **t=188.9s（dur 1.10s、1f0=0.731）**，并写入 F#2–G2 带的复核说明。

## 2026-09-15（续）《向着太阳》两条外部证据实证（测音视频 + 乐评视频）

**测音视频 BV1yLyoByEG4**（UP「一摸提纳-耳测音高」；**无 CC 字幕** → 下载画面读屏上标注）：

| 时刻 | 调性 | 最低音 | 最高音 |
|---|---|---|---|
| t=120s | c minor | **D#2 (78Hz)** | G5 (784Hz) |
| t=240s | c# minor | **D#2 (78Hz)** | **G6 (1568Hz)** |

→ 1568 ÷ 78 = **20.1 倍**（标题「接近20倍赫兹」）、**4.33 个八度**（标题「横贯四个八度」）。
**该视频测的是整首歌（混音 + 两位歌手 + 伴奏），不是王晰个人**（简介写明「原视频出处@王晰的小仓库」）。
78Hz 落在 6 轨分离的 **bass 轨**（伴奏低音器乐）；G6 是 **VITAS 的头声** → **两端都不是王晰的音域**。

**乐评视频 BV1w9K86gEaW**（UP 周恩灿，2026-07-21；ASR 转写 1259 字）关键原话：
- 「超高的**头声**」（VITAS）×「拉到极致的**胸声**」（王晰）——**这首歌的"碰撞"是音色对撞**
- **「花腔我们都知道，花腔炫高音；但是这个花腔难低音、花腔难中低音，还真的没有听到过」**
- 「西哥**大地之神**，非常厚实」；「这个『我』字有**哑音**哦，特别会处理」
- 「史诗级、电影级别的抒情流行歌，很大胆融入跨界古典」
（ASR 更正：花枪→**花腔**、凶声→**胸声**、尖奏→间奏、凭准→音准）

**结论（回答"低音不低似乎不符合这首歌的目的"）**：
本曲的碰撞是**头声高音 × 胸声低音**的音色对撞，真正的技术亮点是**低音区的花腔**（乐评点名为"没听到过"）；
「超低音」听感与外部 78Hz 读数同源于**伴奏低音器乐**（人声轨 60–100Hz 能量仅 0.1497%）。
→ **F#2/G2（91.5–98Hz）完全符合该曲的目的**；按"应该唱到极低"去要求则方向错误。
→ 与 doc2「低音上雕花 / 不刻意刷极限」**独立吻合**（2026-07-21 第三方表述）。

**存档**：`E:\wx\论文素材_王晰作传\原始材料\外部评议\B站_向着太阳_两条外部证据_20260915.md`
（含两段 ASR 转写全文 + 屏上标注记录）
