...
<p class="footnote">试听为 QQ音乐公开试听直链（页面内播放，不下载不托管）；VIP 曲目需登录 QQ音乐。生成时间 {meta["generated_at"]}。</p>
...
(function () {{
  ...
  function cardHtml(s) {{
    ...
    var play = s.mid ? '<button type="button" class="pe-btn" data-mid="' + esc(s.mid) + '">▶ 试听</button>' : '';
  ...
  fetch('data/songs_meta.json', {{ cache: 'no-store' }})
  ...
  input.addEventListener('input', function () {{
    ...
    render(songs.filter(function (s) {{
