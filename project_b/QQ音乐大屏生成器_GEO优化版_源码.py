...
def _compute_radiation(tour_song_fx):
    ...
    #   · 常态型（原「无效型」）：其余（未检出显著正向效应，数据平稳/负向，属常态表现）
    def _pattern(e):
        sl = e.get("setlist_uplift")
        rl = e.get("radiance_uplift")
        if sl is None:
            return "单点触发型" if rl is not None else "常态型"
...
def build_radiation_analysis(tour_song_fx):
    ...
    if cached and cached.get("summary_html") and cached.get("key_findings"):
        ...
        prompt = (
            ...
            "【效应模式分布】各形态中各模式占比(精准型/扩散型/均衡型/出圈型/常态型/单点触发型):\n"
...
<!-- 2026-09-13（晚，按用户决策更正）：dashboard/ 不占主导航位，但**仍对外可见、可被收录**——
     继续进 sitemap、继续提交 IndexNow。此前一度改为 noindex 是误判，已回滚。
     project_b/build_nav.py 的 ARCHIVE_PAGES / ROBOTS_INDEX 守卫做同样的事（幂等双保险）。 -->
...
def _build_analysis(payload):
    ...
    if tour_ps:
        ...
        insights.append({
            "title": "巡演是品牌蓄能期（效应回归区间·非流失）",
            "text": (f"个人巡回 {len(tu)} 个观测中 {pos_ratio}% 为正（均值 {t_avg:+.1f}%），数据形态属「宣发回归的正常区间」——"
