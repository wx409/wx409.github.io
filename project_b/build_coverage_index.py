...
def main() -> int:
    ...
    stat = {"generated_at": datetime.now().isoformat(timespec="seconds"),
            ...
            "measured_shows_site": (lambda sa: sa.get("summary", {}).get("shows_with_material"))(
                jload(DATA / "show_assignment.json", {}) or {}) or
                sum(1 for r in rows if any(e["src"] == "站点实测" for e in r.get("evidence", []))),
            "shows_with_claimable_site": (jload(DATA / "show_assignment.json", {}) or {}).get(
                "summary", {}).get("shows_with_claimable"),
