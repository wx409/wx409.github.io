...
def main():
    ...
    # 0. 指数长表 + 基线刷新（每日一次；失败非致命）
    # ⚠️ 必须排在看门狗**之前**：看门狗会检查「长表新鲜度」，而刷新在 00:04:46 才跑完。
    # 2026-09-12/13/14 连续三天 00:03 的「指数长表滞后 2 天」告警就是这条竞态造成的假告警
    # （看门狗先跑，看到的是刷新前的旧长表）。顺序调换后看门狗看到的是当日新鲜状态。
    if not args.no_index_refresh:
        run_refresh_index()

    # 0b. 看门狗：漏批巡检 + 自愈（2026-09-09 更新重启停摆事故后新增）
