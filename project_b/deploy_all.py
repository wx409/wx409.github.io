...
STEPS = [
    ...
    (ROOT / "tools" / "build_qa_page.py", "问答库静态页 qa.html（实质问答 + FAQPage；需 build_kb_graph 先跑）", True),
    ...
    (ROOT / "project_b" / "build_jazz_page.py", "爵士/Bossa Nova 曲目专区 jazz.html（读 setlists + 专辑实测）", False),
    ...
    (ROOT / "project_b" / "build_album_verify.py", "专辑层复核状态 data/album_verify_status.json（A3 终裁 → per-song/album）", False),
    (ROOT / "project_b" / "build_vocal_longtable.py", "长表与旧源一致性校验（漂移即告警）", False, ["--check"]),
    (ROOT / "project_b" / "inject_long_notes.py", "长声实测注入（从 data/archive_long_notes.json 派生，幂等）", False),
    (ROOT / "project_b" / "inject_top_verified.py", "人耳确认读数注入（listening_verdicts 派生，幂等）", False),
    (ROOT / "project_b" / "audit_extreme_prior.py", "极值先验审计（用该曲已知音域做上限，防假极值上线）", False),
...
INDEXNOW_URLS = [
    ...
    "https://wx409.github.io/songs-shows.html",
    "https://wx409.github.io/jazz.html",
