...
<li style="margin-top:4px"><strong>权威源</strong>：完整口径、逐条复核状态（可主张／待复核／不可复核／已否决）与全量台账见 <a href="/acoustic-report.html">《王晰声学全量报告》</a>；本页只呈现<strong>已确权</strong>读数，口径冲突时以该报告与 <code>data/calibers.md</code> 为准。</li>
</ul>
</div>
<!-- TIER-CALIBER:END -->'''
...
def main() -> None:
    ...
    faq = faq_page([
        ...
        ("现场最低的 B1 读数可信吗？",
         "低音读数只认「最低稳定音」口径：音符本身 ≥0.2s、HNR ≥5dB、强度达标，并须过"
         "「原始混音谐波列完整性」复核（真·单一低音在 1f0–8f0 逐级都在）。"
         "达不到门槛的孤立读数、单帧极值、以及与同场另一路录音相差整数倍的读数，一律列待复核，不纳入结论。"),
    ], url="https://wx409.github.io/stage.html")
    ld_extra = "\n".join(
