...
<li><b>修音敏感分层</b>：稳定性、音准偏差属"<b>高敏感层</b>"，只能组内纵向比；最低音、音区占比、密度、HNR 属"<b>不敏感层</b>"，可跨组比；
<b>颤音速率单列一档「组内可纵比、跨组谨慎」</b>——修音深度改变的是音高轨迹（慢变），而颤音速率是 ~5Hz 的快速调制，
修音**基本不动速率**，但**幅度可能受轻微影响**，故引用速率时优先组内纵向比较。</li>
...
<h3>③ 实证检验：材料级 HNR 能预测"低音读数是否可靠"吗？——<b>不能</b></h3>
<p class="lead">本站既有 <b>149 条 YIN↔CREPE 交叉校验</b>记录（针对最低音候选的**定向复核集**，
...
<b>原始混音谐波列判定 + CREPE 交叉校验 + 人耳终裁</b>三道工序，而不是 HNR；
③ <b>5 dB 是管线内部约定（工程护栏）</b>，本站<b>既未证实、也未证伪</b>它作为"错误阈值"的有效性——
现有定向复核集不足以做随机样本检验，这是本报告公开承认的<b>方法学待办</b>。</p>
...
<p class="lead"><b>两个口径分列，勿混读</b>：<b>按时长</b>保留榜第一＝<b>{f"{_ln_low['dur_s']:.1f} 秒 {_ln_low['note']} {_ln_low['hz']}Hz"}</b>（{esc(_ln_low['src'])}《{esc(_ln_low['song'])}》）；
<b>按音高</b>保留榜最高＝<b>{f"{_ln_high['note']} {_ln_high['hz']}Hz / {_ln_high['dur_s']:.1f} 秒"}</b>（{esc(_ln_high['src'])}《{esc(_ln_high['song'])}》）。
低音每秒气流消耗更大，<b>16.5 秒的 D2 比同时长的高音更考验呼吸管理</b>；而"更高"与"更长"是两件事——
例如录音室《当爱已成往事》B4 496.6Hz 达 12.3 秒，比 C#5 那根 10.3 秒更长，但音高更低。</p>
