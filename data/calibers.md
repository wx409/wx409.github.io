...
> 生成时间：2026-09-21 10:33　｜　生成脚本：`project_b/build_calibers.py`（幂等，部署时自动重跑）
>
...
| `songs_meta` | 歌曲元数据条目 | **593** | 含元信息（时长/专辑/发行等）的曲目 | `data/songs_meta.json` |
...
| `stage_claimable` | 现场层可主张素材 | **294** | 过复核门槛（人耳确认/谐波列通过/双引擎一致）的现场素材条数 | `data/archive_stage_tour.json → summary.n_claimable` |
...
| `kb_facts` | 知识库事实条数 | **1692** | 结构化事实 | `data/kb/manifest.json` |
| `kb_entities` | 知识库实体数 | **1603** | 人/歌/专辑/演出/城市等实体 | `data/kb/manifest.json` |
| `kb_relations` | 知识库关系数 | **2227** | 实体间关系 | `data/kb/manifest.json` |
| `semantic_docs` | 语义索引文档数 | **5417** | 可语义检索的文档块 | `data/kb/semantic/manifest.json` |
...
| `index_days` | 指数数据覆盖天数 | **1294** | 站点图表所用指数长表的实际覆盖天数 | `E:\wx\wx_textmine_out\music_index_long.csv` |
...
| `stage_low_reviewed` | 巡演现场低音读数·谐波列复核通过条数 | **231** | archive_stage_tour.json 中复核状态为「谐波列复核通过」的素材条数 | `data\archive_stage_tour.json` |
| `stage_lowest_hz` | 巡演现场最低稳定音（现行） | **58.2** | 王晰主导巡演现场·过复核门槛的最低稳定音（Hz） | `data\archive_stage_tour.json` |
