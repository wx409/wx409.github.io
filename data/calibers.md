...
> 生成时间：2026-09-23 00:11　｜　生成脚本：`project_b/build_calibers.py`（幂等，部署时自动重跑）
>
...
| `stage_claimable` | 现场层可主张素材 | **475** | 过复核门槛（人耳确认/谐波列通过/双引擎一致）的现场素材条数 | `data/archive_stage_tour.json → summary.n_claimable` |
...
| `stage_low_reviewed` | 巡演现场低音读数·谐波列复核通过条数 | **424** | archive_stage_tour.json 中复核状态为「谐波列复核通过」的素材条数 | `data\archive_stage_tour.json` |
