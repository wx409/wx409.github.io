...
> 生成时间：2026-09-11 09:02　｜　生成脚本：`project_b/build_calibers.py`（幂等，部署时自动重跑）
>
...
| `songs_meta` | 歌曲元数据条目 | **587** | 含元信息（时长/专辑/发行等）的曲目 | `data/songs_meta.json` |
...
| `index_days` | 指数数据覆盖天数 | **1283** | 站点图表所用指数长表的实际覆盖天数 | `E:\wx\wx_textmine_out\music_index_long.csv` |
