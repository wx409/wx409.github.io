...
> 生成时间：2026-09-20 21:02　｜　生成脚本：`project_b/build_calibers.py`（幂等，部署时自动重跑）
>
...
| `kb_entities` | 知识库实体数 | **1602** | 人/歌/专辑/演出/城市等实体 | `data/kb/manifest.json` |
| `kb_relations` | 知识库关系数 | **2226** | 实体间关系 | `data/kb/manifest.json` |
| `semantic_docs` | 语义索引文档数 | **5415** | 可语义检索的文档块 | `data/kb/semantic/manifest.json` |
...
| `index_days` | 指数数据覆盖天数 | **1293** | 站点图表所用指数长表的实际覆盖天数 | `E:\wx\wx_textmine_out\music_index_long.csv` |
...
| `stage_low_reviewed` | 巡演现场低音读数·谐波列复核通过条数 | **171** | archive_stage_tour.json 中复核状态为「谐波列复核通过」的素材条数 | `data\archive_stage_tour.json` |
